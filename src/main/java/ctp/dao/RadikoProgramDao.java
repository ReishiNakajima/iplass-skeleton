...
public class RadikoProgramDao extends Dao<RadikoProgram> {
	...
	public List<RadikoProgram> findBetweenStartDate(Timestamp fromDate, Timestamp toDate) {
		...
		query.select(
				...
				// StringUtils.joinWith(DOT, RadikoProgram.RADIKO_STATION,
				// RadikoStation.CALL_SIGN),
				// StringUtils.joinWith(DOT, RadikoProgram.RADIKO_STATION,
				// RadikoStation.STATION_NAME),
				RadikoProgram.PROGRAM_NAME, RadikoProgram.START_DATETIME, RadikoProgram.NOTE, RadikoProgram.RADIKO_URL,
				RadikoProgram.DEADLINE, RadikoProgram.LISTEN_STATUS)
